        ...
        <div class="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-slate-800">
                    <tr>
                        <th scope="col"
                            class="px-6 py-3 text-left text-xs font-bold text-gray-300 uppercase tracking-wider">Course
                            Name</th>
                        <th scope="col"
                    ...
                    @foreach($courses as $course)
                        <tr class="hover:bg-slate-50 transition duration-150 ease-in-out">
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    <div
                                        class="flex-shrink-0 h-10 w-10 rounded-lg bg-indigo-100 flex items-center justify-center text-indigo-600 font-bold border border-indigo-200">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-xs text-slate-500">
                                    <div><span class="font-bold text-slate-700">Start:</span>
                                        {{ $course->start_date ? \Carbon\Carbon::parse($course->start_date)->format('M d, Y') : 'TBA' }}
                                    </div>
                                    <div><span class="font-bold text-slate-700">End:</span>
                                        {{ $course->end_date ? \Carbon\Carbon::parse($course->end_date)->format('M d, Y') : 'TBA' }}
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                                    class="px-2.5 py-0.5 rounded-md text-xs font-medium bg-slate-100 text-slate-700 border border-slate-200">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                ...
                                <form action="{{ route('courses.destroy', $course->course_id) }}" method="POST"
                                    class="inline-block"
                                    onsubmit="return confirm('Delete this course? All student enrollments for this course will also be deleted.');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
